...
static inline void calc_cpu_hotplug(unsigned int counter0, unsigned int counter1)
{
...
static void __cpuinit awifi_hotplug_early_suspend(struct early_suspend *handler)
{	 
    int cpu;
...
static void __cpuinit awifi_hotplug_late_resume(struct early_suspend *handler)
{  
    int cpu;
...
static struct early_suspend awifi_hotplug_suspend =
{
    .level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1,
	.suspend = awifi_hotplug_early_suspend,
	.resume = awifi_hotplug_late_resume,
};
...
int __init awifi_hotplug_init(void)
{
	pr_info("AWIFI Hotplug driver started.\n");
    ...
    wq = alloc_workqueue("awifi_hotplug_workqueue", 
    ...
    register_early_suspend(&awifi_hotplug_suspend);
    
    return 0;
}
late_initcall(awifi_hotplug_init);
