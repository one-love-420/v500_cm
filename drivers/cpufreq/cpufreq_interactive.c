...
rearm:
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		pcpu->time_in_idle = get_cpu_idle_time(data, &pcpu->idle_exit_time);
		mod_timer_pinned(&pcpu->cpu_timer, jiffies + usecs_to_jiffies(timer_rate));
	}
...
static void cpufreq_interactive_idle_start(void)
{
	...
	if (pcpu->target_freq != pcpu->policy->min) {
		 ...
		 */
		if (!pending) {
			pcpu->time_in_idle = get_cpu_idle_time(smp_processor_id(), &pcpu->idle_exit_time);
...
static void cpufreq_interactive_freq_down(struct work_struct *work)
{
	...
	for_each_cpu(cpu, &tmp_mask) {
		...
		for_each_cpu(j, pcpu->policy->cpus) {
			struct cpufreq_interactive_cpuinfo *pjcpu = &per_cpu(cpuinfo, j);
...
static ssize_t store_hispeed_freq(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	int ret;
	u64 val;
...
static struct global_attr hispeed_freq_attr = __ATTR(hispeed_freq, 0644, show_hispeed_freq, store_hispeed_freq);


static ssize_t show_up_threshold(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%lu\n", up_threshold);
}

static ssize_t store_up_threshold(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static struct global_attr up_threshold_attr = __ATTR(up_threshold, 0644, show_up_threshold, store_up_threshold);
...
static struct global_attr min_sample_time_attr = __ATTR(min_sample_time, 0644, show_min_sample_time, store_min_sample_time);

static ssize_t show_above_hispeed_delay(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%lu\n", above_hispeed_delay_val);
}

static ssize_t store_above_hispeed_delay(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_rate(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%lu\n", timer_rate);
}

static ssize_t store_timer_rate(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static struct global_attr timer_rate_attr = __ATTR(timer_rate, 0644, show_timer_rate, store_timer_rate);

static ssize_t show_input_boost_freq(struct kobject *kobj, struct attribute *attr, char *buf)
{
...
static struct global_attr dynamic_scaling_attr = __ATTR(dynamic_scaling, 0644, show_dynamic_scaling, store_dynamic_scaling);
...
static int cpufreq_governor_interactive(struct cpufreq_policy *policy, unsigned int event)
{
	...
	for_each_cpu(j, policy->cpus) {
                ...
                pcpu->target_set_time_in_idle = get_cpu_idle_time(j, &pcpu->target_set_time);
                pcpu->floor_freq = pcpu->target_freq;
                pcpu->floor_validate_time = pcpu->target_set_time;
