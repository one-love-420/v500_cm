...
static void cpufreq_interactive_timer(unsigned long data)
{
	...
	/* we want cpu0 to be the only core blocked for freq changes while we are touching the screen for UI interaction */
...
static struct global_attr go_hispeed_load_attr = __ATTR(up_threshold, 0644, show_up_threshold, store_up_threshold);
...
static ssize_t show_dynamic_scaling(struct kobject *kobj, struct attribute *attr,
			  char *buf)
{
	return sprintf(buf, "%d\n", dynamic_scaling);
}

static ssize_t store_dynamic_scaling(struct kobject *kobj, struct attribute *attr,
			   const char *buf, size_t count)
{
	int ret;
	unsigned long val;
